...
function ActivityCard({
  activity, score, selected, disabled, usedForOtherRubric, keywords, generatedBullet, onToggle,
}: {
  activity: Activity; score: number; selected: boolean; disabled: boolean;
  usedForOtherRubric: boolean; keywords: string[];
  generatedBullet?: GeneratedBullet; onToggle: () => void;
}) {
  return (
    <div className={`rounded-xl border transition-all ${
      selected            ? "border-zinc-900 ring-2 ring-zinc-900 bg-zinc-50" :
      usedForOtherRubric  ? "border-zinc-200 bg-zinc-50 opacity-50 cursor-not-allowed" :
      ...
      <div
        onClick={!disabled && !usedForOtherRubric ? onToggle : undefined}
        className={`flex items-center gap-2 px-3 pt-3 pb-2 select-none ${!disabled && !usedForOtherRubric ? "cursor-pointer" : ""}`}
      >
        ...
        {usedForOtherRubric ? (
          <span className="text-xs text-zinc-400 italic shrink-0">used</span>
        ) : (
...
function AddActivityForm({
  keywords, draft, onDraftChange, onSubmit, onCancel,
}: {
...
export default function MatchReview({
  atsRubric, knockoutItems, matches, activities, selections,
  ...
  const atMax          = uniqueCount >= MAX_ACTIVITIES;
  ...
  const allMatched     = completedCount === sorted.length && sorted.length > 0;
  ...
  function setPanelRef(rubricId: string) {
    return (el: HTMLDivElement | null) => {
      panelRefs.current[rubricId] = el;
    };
  }

  function scrollToPanel(rubricId: string) {
    setOpenPanels((prev) => ({ ...prev, [rubricId]: true }));
    // Use rAF to let the panel expand before scrolling
  ...
  useEffect(() => {
    ...
    if (firstUnmatched) {
      ...
      // All matched — scroll to last item to signal completion
    ...
    <div className="flex flex-col gap-4 max-w-2xl">
      ...
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm rounded-xl border border-zinc-200 shadow-sm p-4 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-zinc-900">
              ...
              {uniqueCount > 0 && (
                <span className={`ml-2 font-normal text-xs ${uniqueCount < MIN_ACTIVITIES ? "text-amber-600" : "text-green-600"}`}>
        ...
        <div className="flex items-center gap-4 text-xs text-zinc-400">
          ...
          {atMax && (
            <span className="ml-auto text-amber-600 font-medium">Max {MAX_ACTIVITIES} activities reached</span>
          )}
        </div>
      </div>

      {/* Knockout items (informational) */}
      {knockoutItems && knockoutItems.length > 0 && (
        <details className="group bg-white rounded-xl border border-amber-200 overflow-hidden">
          <summary className="flex items-center gap-2 px-4 py-3 cursor-pointer select-none list-none">
            <span className="text-amber-600 text-sm">⚠</span>
            <span className="text-sm font-semibold text-zinc-900 flex-1">Knockout requirements to check</span>
            <span className="text-xs text-zinc-400">{knockoutItems.length} item{knockoutItems.length !== 1 ? "s" : ""}</span>
          ...
          <div className="border-t border-amber-100 px-4 pb-3 pt-3 flex flex-col gap-2">
            <p className="text-xs text-zinc-500">These are hard requirements from the job posting. Make sure you qualify before proceeding.</p>
            {knockoutItems.map((ko) => (
              <div key={ko.item_id} className="flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-100 px-3 py-2">
                <span className="text-sm shrink-0 mt-0.5">{CATEGORY_ICONS[ko.category] ?? "⚠️"}</span>
                <p className="text-sm text-zinc-700">{ko.requirement}</p>
              </div>
            ))}
          </div>
        </details>
      ...
      {sorted.map((item) => {
        ...
        const hasMatches    = (matches[item.rubric_id] ?? []).length > 0;

        // Border color: green = matched, amber = unmatched + has matches, default = no matches
        ...
        const itemMatches = [...(matches[item.rubric_id] ?? [])]
          .sort((a, b) => {
        ...
        return (
          ...
          >
            ...
            >
              <span className={`rounded-md px-2 py-0.5 text-xs font-medium shrink-0 ${PRIORITY_COLORS[item.priority] ?? PRIORITY_COLORS.nice_to_have}`}>
                {PRIORITY_LABELS[item.priority] ?? item.priority}
              </span>
              <span className="flex-1 text-sm font-semibold text-zinc-900 text-left leading-snug">{item.item}</span>
              {hasSelections ? (
                <span className="text-xs text-green-600 font-medium shrink-0">✓ {selectedIds.length}</span>
              ) : hasMatches ? (
                <span className="text-xs text-amber-500 font-medium shrink-0">Review needed</span>
              ) : null}
              <svg className={`w-4 h-4 text-zinc-400 shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 16 16">
                <path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
            ...
            {isOpen && (
              <div className="border-t border-zinc-100 px-4 pb-4 pt-3 flex flex-col gap-3">
                ...
                {itemMatches.length === 0 ? (
                  <p className="text-sm text-zinc-400 italic">No matching activities found.</p>
                ) : (
                  <div className="flex flex-col gap-2">
                    {itemMatches.map((vm) => {
                      ...
                      const isUsedForOther    = !isSelected && allSelectedIds.has(vm.bullet_id);
                      ...
                      return (
                        <ActivityCard
                          ...
                          selected={isSelected}
                          disabled={isDisabled}
                          usedForOtherRubric={isUsedForOther}
                          keywords={keywords}
